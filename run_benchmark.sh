...
do
   ...
   do
      ...
      do
        ...
        echo "\nData for run $((i+=1)), -bpred ${bpredm} -res:fpalu 1 -lsq:size ${lsqs} -ruu:size ${ruus} cache mode l" >> total_log.txt
      ...
      echo "\nData for run $((i+=1)), -bpred ${bpredm} -res:fpalu 1 -lsq:size ${lsqs} -ruu:size 32  cache mode f" >> total_log.txt
      grep "Total Power Consumption" tmp_log.txt >> total_log.txt
      grep "sim_IPC" tmp_log.txt >> total_log.txt
      grep "sim_CPI" tmp_log.txt >> total_log.txt
      grep "\.misses " tmp_log.txt | grep -v "bpred" >> total_log.txt
